...
class AddConsolidationCenter extends React.Component {
  ...
  getServiceList(centerId) {
    try {
      ...
      api
        .post("userManagement/getServiceRateList", data)
        .then((res) => {
          if (res.success) {
  ...
  getListData(centerId) {
    try {
      ...
      api
        .post("userManagement/getConsolidationEditData", data)
        .then((res) => {
          if (res.success) {
            api
              .get("location/getCountryList")
              .then((res1) => {
                if (res1.success) {
                  var labeldata = res1.data.filter(
                    (x) => x.CountryID === res.data.CountryId
                  );
                  var data = {
                  ...
                  this.setState({
                    ...
                    CenterName:
                      res.data.CenterName === null ? "" : res.data.CenterName,
                    CompanyName:
                      res.data.CompanyName === null ? "" : res.data.CompanyName,
                    AddressLine1:
                      res.data.AddressLine1 === null
                        ? ""
                        : res.data.AddressLine1,
                    AddressLine2:
                      res.data.AddressLine2 === null
                        ? ""
                        : res.data.AddressLine2,
                    Country: data,
                    ZipCode: res.data.ZipCode,
                    CityInput: res.data.City,
  ...
  handleChange = (event, type) => {
    if (type === "centername") {
      ...
      this.setState({ CityInput: cityValue });
      console.log("cityDattaValuee", event.target.value);
    } else if (type === "State") {
  ...
  saveCenter = (redirect) => {
    if (this.validate()) {
      try {
        ...
        api
          .post("userManagement/addUpdateConsolidation", UserDetails)
          .then((res) => {
            if (res.success) {
  ...
  DeleteCenter = (redirect) => {
    if (this.validate()) {
      try {
        ...
        api
          .post("userManagement/DeleteConsolidationCenter", UserDetails)
          .then((res) => {
            if (res.success) {
              this.props.history.push({
                pathname: "/admin/ConsolidationCenterList",
  ...
  saveService = (redirect) => {
    ...
    if (this.validate()) {
      try {
        ...
        api
          .post("userManagement/addUpdateServiceRate", Details)
          .then((res) => {
            if (res.success) {
              this.props.history.push({
                pathname: "/admin/ConsolidationCenterList",
    ...
    try {
      ...
      var callApi =
        this.state.isClearance === true
          ? "userManagement/UpdateClearancecharges"
          : "userManagement/UpdateTransportcharges";
      api
        .post(callApi, Details)
        .then((res) => {
          if (res.success) {
            this.props.history.push({
              pathname: "/admin/ConsolidationCenterList",
    ...
    try {
      ...
      api
        ...
        .then((res) => {
          if (res.success) {
            this.setState({
              ConsolidationCenterDataList: res.data,
              Loading: false,
            });
          } else {
            cogoToast.error("Something Went Wrong");
  ...
  editService = (record) => {
    var labeldata = this.state.ConsolidationCenterDataList.filter(
      (x) => x.ID === record.original.ConsolidationId
    );
    var data = {
  ...
  getChargeList(input) {
    try {
      ...
      this.setState({ Loading: true });
      api
        .post("userManagement/getChargesList", data)
        .then((res) => {
          if (res.success) {
  ...
  editCharges = (record) => {
    ...
    this.setState({
      ...
      isClearance:
        record.original.ServiceName === "Clearance Charges" ? true : false,
    });
  };
  ...
  activeInactiveService = (record) => {
    ...
    try {
      this.setState({ Loading: true });
      api
        .post("userManagement/activeInactiveServiceRate", data)
        .then((res) => {
          if (res.success) {
  ...
  renderMarkup = () => {
    if (this.state.isClearance === true) {
      return this.state.chargeList.map((service) => {
        const { ChargeId, ChargeName, Rate, RateType, Type } = service;
          ...
          <tr key={ChargeId}>
            <td>
              <input
                ...
                onBlur={(e) => this.handleBlur(e, ChargeId, "ChargeName")}
              />
            </td>
            <td>
              <input
                ...
                className="form-control"
                value={Rate}
                onChange={(event) => this.handledInput(event, ChargeId, "Rate")}
                onBlur={(e) => this.handleBlur(e, ChargeId, "Rate")}
              />
            </td>
            <td>
              <input
                ...
                onBlur={(e) => this.handleBlur(e, ChargeId, "RateType")}
              />
            </td>
            <td>
              <input
                ...
                className="form-control"
                value={Type}
                onChange={(event) => this.handledInput(event, ChargeId, "Type")}
                onBlur={(e) => this.handleBlur(e, ChargeId, "Type")}
              />
            </td>
          </tr>
        );
      ...
      return this.state.chargeList.map((service) => {
        const { ChargeId, StartCFT, Rate, EndCFT } = service;
          ...
          <tr key={ChargeId}>
            <td>
              <input
                ...
                onBlur={(e) => this.handleBlur(e, ChargeId, "StartCFT")}
              />
            </td>
            <td>
              <input
                ...
                onBlur={(e) => this.handleBlur(e, ChargeId, "EndCFT")}
              />
            </td>
            <td>
              <input
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        Cell: (record) => {
          if (
            record.original.ServiceName === "Clearance Charges" ||
            record.original.ServiceName === "Transportation Charges"
          ) {
            return (
            ...
            return <div>{record.original.Rate}</div>;
          }
        },
      },
      {
    ...
    const ConsolidationCenterList = this.state.ConsolidationCenterDataList.map(
      (type) => {
            ...
            <Card>
              ...
              <Cardbody>
                ...
                <div className="shipment-content">
                  <div
                    className="shipment-pane mt-20"
                    id="addconsolidationcenter"
                  >
                    {/* <Card> */}
                    {/* <CardBody> */}
                    <GridContainer>
                    ...
                    <GridContainer>
                      ...
                      <GridItem xs={12} sm={12} md={3}>
                        ...
                        {!CityInput || this.state.CityAutoComplete === false ? (
                          (console.log("CheckCityErrorValue", CityInput),
                          console.log(
                            "CheckCityErrorValueF",
                            this.state.CityAutoComplete
                          ),
                          console.log("CheckCityErrorValueC", City),
                          (
                            <CustomInput
                              labelText="City"
                              id="city"
                              ...
                              inputProps={{
                                ...
                                onChange: (event) =>
                                  this.handleChange(event, "City"),
                                // value: CityInput,
                                value: City?.value ? City?.value : CityInput,
                                endAdornment: (
                                  <InputAdornment
                                    position="end"
                    ...
                    <div className="shipment-submit">
                      <div className="left">
                        {this.props.location.state ? (
                          <Button
                            color="danger"
                            // onClick={() => this.DeleteCenter(false)}
                            onClick={() => this.openDeleteRequestModalKeyword()}
                          >
                            Delete
                          </Button>
                        ) : null}
                        ...
                        <Card>
                          ...
                          <div className="right">
                            <Button
                              ...
                              onClick={() =>
                                this.AddService(
                                  this.state.ConsolidationCenterId
                                )
                              }
                            >
                              Add
                      ...
                      <Card>
                        ...
                        {this.props.location.state ? null : (
                          <div className="right">
                            <Button
                              color="rose"
                              onClick={() => this.saveService(false)}
                            >
                              Save
                            </Button>
                            <Button
                    ...
                    <GridItem xs={12} sm={12} md={3}>
                      <CustomInput
                        ...
                        inputProps={{
                          ...
                          onBlur: (event) => this.handleChange(event, "rate"),
            ...
            <div>
              ...
              >
                ...
                <DialogActions>
                  ...
                  <Button
                    color="secondary"
                    onClick={() => this.setState({ Serviceopen: false })}
                  >
                    Cancel
                  </Button>
                </DialogActions>
