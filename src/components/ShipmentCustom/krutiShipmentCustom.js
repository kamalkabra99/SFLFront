...
class ShipmentCustom extends React.Component {
    ...
    try {
      ...
      api.post("scheduleshipment/getShipmentInfo", data).then((res) => {
        if (res.success) {
          ...
          if (
            res.data[0].ContainerID !== null ||
            res.data[0].ContainerID !== ""
          ) {
            this.containerOnBlur("", res.data[0].ContainerID, "");
          }
  ...
  containerOnBlur = (event, value, type) => {
    debugger;
    try {
      let data = {
        containerID: value.value ? value.value : value,
      };

      api
        .post("container/getMaxSequenceIdByContainer", data)
        .then((res) => {
          debugger;
          if (res.success) {
            console.log("resassss", res.data[0][0].maxSequenceId);
            this.setState({ maxSequenceId: res.data[0][0].maxSequenceId });
          }
        })
        .catch((err) => {
          console.log("err...", err);
        ...
        if (this.state.AllClear !== value.value) {
          ...
          if (
            ...
            if (
              ...
              value.value === "Yes" &&
              this.state.TotalCostInvoice != this.state.TotalCostReceived
            ) {
              this.setState({ IsChanged: false });
  ...
  addPackageRow = () => {
    ...
    var row = {
      ...
      Sequence: this.state.maxSequenceId + (this.state.PackageList.length + 1), //kruti
