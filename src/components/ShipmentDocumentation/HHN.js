...
class HHNInvoice extends Component {
	...
	if(size>1){
		for (let index = 0; index < size; index++) {
			...
			var data = index+1
				...
				this.state.multipleArray.push(newarrayobj)
		}
	}
  ...
  getDatatable = (element) =>{
	...
	var enddata = 0;
	var recordend = element*15;
	var startdata = (recordend-15) + 1;
	var mapPush = []

	// if(recordend > this.state.htmlData.length){
	// 	recordend = this.state.htmlData.length
	// }

	for (let index = startdata; index <= recordend; index++) {
		if(this.state.htmlData[index-1] != undefined){
			mapPush.push(this.state.htmlData[index-1])
		}
		else{
			var newData = {
				PackageNumber:"",
			...
			mapPush.push(newData)
		}
		

	}

	console.log("mapPush = ",mapPush)

	return mapPush.map((commercial, idx) => {
		return (
		  <tr>
  ...
  getdata = () =>{
	...
	return this.state.multipleArray.map((commercial, idx) =>{
		return (
			...
			<div className="hg-table-container">
				...
				<table className="common-table bordered-table mt-20">
					...
					{
						this.getDatatable(commercial.indexdata)
					}
				...
				<table className="common-table bordered-table mt-20 font-13 marginBottom-table">
					<tr>
  ...
  render() {
    ...
    return (
        <div className="hg-table">
			{/* <p>Hello {this.state.sizelen}</p> */}
			{this.state.sizelen > 1?(
