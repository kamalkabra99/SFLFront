...
class ResourceAllocationList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ProjectList:[],
  ...
  handleDelete = (record) => {debugger
    ...
    if (CommonConfig.getUserAccess("Resource Allocation List").AllAccess === 1) {
      api.post("projectManagement/deleteResourceAllocation", data).then((res) => {console.log(res);
        if(res.message.data[0][0].DeleteRes == 0)
          cogoToast.error("This Resource of Service is used somewhere");
        else
        this.getResourceByProject(this.state.ProjectID,0);
  ...
  selectChange = (event, value, type) => {debugger
    ...
    if (value !== null) {
      ...
      if (type === "ServiceName") {
        ...
        this.setState({ServiceName: selectData,ServiceID: value.value });
        this.getResourceByProject( this.state.ProjectID,value.value);
        // /this.getServicesByProject( value.value,selectData);
      }
    }
    else{
      if(this.state.ProjectID !="" && this.state.ProjectID !=undefined)
    this.getResourceByProject( this.state.ProjectID,value);}
  };
            ...
            <div className="table-common-btn">
              ...
              1 ? (
                <Button
                  ...
                  color="danger"
                  onClick={() =>
                    this.openDeleteRequestModalKeyword(record.original.ServiceResourceID)
                  }
                >
      ...
      <GridContainer className="UserList-outer">
        ...
        <GridItem xs={12}>
          <Card>
            ...
            <CardBody>
              <ReactTable
                data={ProjectServiceResourceList}
                defaultPageSize={10}
                minRows={2}
