...
import { classes, useStyles, valid } from "./EditSalesLeads";

export class EditSalesLeads extends Component {
  constructor(props) {
    super(props);
    this.state = {
      DallasTotal: 0,
  ...
  async GetRateData() {
    ...
    //kkkkk
    if (this.state.fromStateList.length > 0) {
    ...
    UpsData.FromStateProvinceCode = fromStateCcode
      ? fromStateCcode[0].StateCode
      : "";
    ...
    if (propsalType === "Air") {
      ...
      if (res.success) {
        ...
        this.state.finalGetResults.forEach((item) => {
  ...
  async confirmPickup() {
    ...
    } else if (this.state.pickUpValue === "FEDEX" && this.state.furniture > 0) {
      this.setState({ PickupOpenPopup: false });
      this.hideLoader();
      return cogoToast.error("pickup type for Furniture fedex not avaliable ");
    } else {
